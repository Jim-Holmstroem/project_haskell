...
potential_tree = ( scanl1 potential_up ) . reverse

load_matrix :: String -> [[Int]]
load_matrix = (map ((map read).(splitOn " "))).(filter (not.null)).(splitOn "\r\n") --since the input file is in windows format we need \r\n

--main = print  potential_tree --tip of the potential tree is the answer: sum-only, the root is totally lost
main = do
    contents <- getContents  --load data
    print.head.last.potential_tree.load_matrix $ contents --parse data
